...
const Selector = require("./grammar-selector");
...
(Highlights = class Highlights {
    ...
    constructor(param) {
        ...
        if (this.registry == null) {
            this.registry = new GrammarRegistry({maxTokensPerLine: Infinity});
        }
    ...
    loadGrammarsSync() {
        ...
        if (typeof this.includePath === "string") {
            if (fs.isFileSync(this.includePath)) {
                ...
                for (let filePath of Array.from(fs.listSync(this.includePath, ["cson", "json"]))) {
    ...
    _populateGrammars(grammarsFromJSON, grammarsArray, cb) {
        ...
        const done = (err, grammar) => {
            ...
            if (!--toLoad) {
                ...
                for (let grammarPath in grammarsFromJSON) {
        ...
        if (typeof this.includePath === "string") {
            return fs.stat(this.includePath, (err, stat) => {
                ...
                if (stat.isFile()) {
                    ...
                    return fs.list(this.includePath, ["cson", "json"], function(err, list) {
                        if (list == null) {
                            list = [];
                        } return cb(err, list);
                    });
